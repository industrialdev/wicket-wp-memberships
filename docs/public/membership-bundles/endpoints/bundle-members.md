...
## [Add a member to a bundle](#add-a-member-to-a-bundle)
...
## [Remove a member from a bundle](#remove-a-member-from-a-bundle)
...
## [Move a member to another bundle](#move-a-member-to-another-bundle)
...
> **Important:** There is no rollback if the source membership is successfully cancelled but the target creation fails. The error message will explicitly note this. Use `add_member` with `mode: "new"` to re-add the member to a bundle manually.
...
## [Change the bundle owner](#change-the-bundle-owner)
