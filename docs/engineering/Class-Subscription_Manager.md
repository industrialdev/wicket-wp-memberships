...
- `terminate_at_membership_end($sub, $end_ts)` — Clamps a subscription's `end` to a membership's end date and **deletes** a `next_payment` falling on or after it. For a membership renewed into a *new* subscription, where the old one would otherwise keep billing to its own later `end`. Deliberately does not route through `prepare_dates()`: that guard preserves a colliding `next_payment` by nudging it an hour earlier, which here would take one more payment the member no longer owes — the intent is the opposite. Only ever shortens, skips (with a `wc_log_mship_error`) when the status forbids date changes or the target `end` precedes the last payment, and catches the `update_dates()` exception. Added for the monthly autopay Form Flow repair — see [that bug fix doc](../bugfix/monthly-autopay-form-flow-renewal-callout.md).
...
- [Monthly Autopay Form Flow — Hidden Renewal Callout & Orphaned Subscription](../bugfix/monthly-autopay-form-flow-renewal-callout.md) — the repair that added `terminate_at_membership_end()`
