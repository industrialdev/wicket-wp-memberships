...
- `drop_superseded_next_payment($sub)` — Removes `next_payment` from a subscription whose membership has been renewed onto a *new* subscription, which otherwise leaves the old one with a live payment date and nothing to stop it. With the date gone the subscription is inert: no further payment, expires on its own end date without renewing. **Monthly billing is excluded** — those payments are instalments against a term the member has already taken, so they are still owed. Touches nothing else: end date, status and autopay flag are left as they were. Deliberately does not route through `prepare_dates()`, which exists to *preserve* a colliding `next_payment`; here the payment is what is being removed. Added by the [monthly Form Flow repair](../bugfix/monthly-autopay-form-flow-renewal-callout.md) as `terminate_at_membership_end()`, which also clamped `end`; reduced to this single operation and renamed by the [annual follow-on](../bugfix/annual-autopay-form-flow-renewal-callout.md).

**Adjustment size — 1 hour (`NEXT_PAYMENT_COLLISION_OFFSET`).** Originally 1 second; widened after investigation. WC Subscriptions' payment/retry logic gates on subscription *status*, not on whether `end` has been reached — a failed renewal charge goes `on-hold` and enters a ~7-day dunning cycle, and that status change unschedules the independent `end`-date expiration action. However, `next_payment` (charge trigger) and `end` (expiration trigger) are two *independently scheduled* Action Scheduler jobs; if the charge-processing action is delayed (gateway latency, queue congestion) and the `end` job fires first while status is still `active`, `expire_subscription()` unconditionally flips the subscription to `expired` with no check of in-flight payment attempts. A 1-second gap left essentially no buffer against this race. An hour costs nothing functionally (the only enforced invariant is `end > next_payment`, any positive gap satisfies WC's validation), removes most of the exposure, keeps the two dates visibly distinct in the admin UI (which renders to the minute rather than the second), and stays inside the same calendar day so the nudge can't roll `next_payment` back onto the previous date. Widening further (e.g. a full day) would break that last property for day-boundary `end` values.
...
- [Monthly Autopay Form Flow — Hidden Renewal Callout & Orphaned Subscription](../bugfix/monthly-autopay-form-flow-renewal-callout.md) — the repair that added this method
- [Annual Autopay Form Flow — Callout Still Hidden & Term-End Auto Payment](../bugfix/annual-autopay-form-flow-renewal-callout.md) — the follow-on that reduced it to `drop_superseded_next_payment()`
- [Monthly Subscription Inherits a Sibling Annual Subscription's Next Payment Date](../bugfix/monthly-subscription-inherits-annual-next-payment-date.md) — WWID-2425: `prepare_dates()` output (`end − 1h`, produced by a grace-0 collision) was replayed onto the *wrong* subscription by an unbound deferred closure in `Membership_Controller`, and passed WC's validation precisely *because* it had already been nudged. Fixed by binding both deferred closures to their target subscription id; `prepare_dates()` itself was correct and unchanged
