...
# Membership_Group_Config Class Index

**File:** includes/Membership_Group_Config.php

Represents a Membership Group Config CPT record (`wicket_mship_grp_cfg`). Combines the date/cycle/renewal-window logic of `Membership_Config` with the approval and renewal-type logic of `Membership_Tier`, scoped specifically for Membership Groups. Renewal type is limited to `subscription` and `form_page`.
...
**__construct($post_id)**
Validates the post exists and is the `wicket_mship_grp_cfg` CPT type. On failure, sets `$post_id = 0` and empties all data properties (logs via `Wicket()->log()`). On success, loads all four meta arrays.
