...
**Error handling:** items with missing `user_id`, a `sequential_logic` member with no charge meta (fallback does not apply), or no tier resolvable at all, are skipped and logged. Failed `add_member()` calls are logged and recorded in the `errors` array in the completion meta.
