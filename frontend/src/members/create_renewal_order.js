...
import { ErrorsRow, BorderedBox, ActionRow, LabelWpStyled, SelectWpStyled } from '../styled_elements';
import { Button, Flex, FlexItem, FlexBlock, Notice, Icon, Modal } from '@wordpress/components';
import styled from 'styled-components';
import { fetchWcProducts, fetchProductVariations, createRenewalOrder } from '../services/api';
...
const CreateRenewalOrder = ({ membership }) => {
  ...
  const [isCreateRenewalOrderModalOpen, setIsCreateRenewalOrderModalOpen] = useState(false);
  ...
  const getSelectedProductId = () => {
    ...
    if (createRenewalOrderFormData.product_id === null) { return null; }
  ...
  const getSelectedProductVariationId = () => {
    ...
    if (createRenewalOrderFormData.variation_id === null) { return null; }
  ...
  /**
	...
	const getSelectedCreateRenewalOrderProductType = () => {
		...
		const product = wcProductOptions.find(option => option.value === createRenewalOrderFormData.product_id);
      ...
      <BorderedBox>
        <div style={{ textAlign: 'left' }} >
          ...
          {createRenewalOrderFormData.order_link !== null && (
            <Notice isDismissible={false} status="success">{__('Renewal Order created successfully.', 'wicket-memberships')} <Button variant='link' href={createRenewalOrderFormData.order_link} target="_blank">{__('View Order', 'wicket-memberships')}</Button>&nbsp;<Icon icon='external' style={{ color: 'var(--wp-admin-theme-color)' }} /></Notice>
          )}
              ...
              >
              <FlexBlock>
                <LabelWpStyled htmlFor={'create_renewal_order_product_id'}>{__('Product', 'wicket-memberships')}</LabelWpStyled>
                <SelectWpStyled
            ...
            {getSelectedCreateRenewalOrderProductType() === 'variable-subscription' && (
              <MarginedFlex>
                <FlexBlock>
                <LabelWpStyled htmlFor={'create_order_renewal_variation_id'}>{__('Variable', 'wicket-memberships')}</LabelWpStyled>
                <SelectWpStyled
