...
const validate = (form) => {
  ...
  if (!form.bundleConfig) {
    errors.bundleConfig = __("Bundle config is required.", "wicket-memberships");
  }
...
/**
 * CreateMembershipBundlePage — form for creating a new Membership Bundle post.
 *
 * @param {object} props
 * @param {string} props.bundleConfigCptSlug  - CPT slug for bundle configs (from data attribute).
 * @param {string} props.listUrl             - URL of the membership bundle list page.
 * @param {string} props.editBundleBaseUrl    - Base URL for the bundle edit page (id appended on success).
 */
const CreateMembershipBundlePageContent = ({ bundleConfigCptSlug, listUrl, editBundleBaseUrl }) => {
  ...
  const loadBundleConfigs = async () => {
    const posts = await apiFetch({
      path: addQueryArgs(`${API_URL}/${bundleConfigCptSlug}`, {
  ...
  const loadOwnerOptions = (inputValue, callback) => {
    ...
    fetchMdpPersons({ term: inputValue })
      ...
      .catch((error) => {
        console.error("[CreateMembershipBundlePage] loadOwnerOptions error", error);
      });
  };
  ...
  const loadOrgOptions = useCallback((inputValue, callback) => {
    ...
    fetchSearchOrgs(inputValue)
      ...
      .catch((error) => {
        console.error("[CreateMembershipBundlePage] loadOrgOptions error", error);
        callback([]);
      });
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await createMembershipBundle({
        name:                       form.name.trim(),
        membership_bundle_config_id: form.bundleConfig.value,
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: "16px" }}>
          <TextControl
            ...
            placeholder={__("Membership Bundle Name", "wicket-memberships")}
            value={form.name}
            onChange={set("name")}
          />
        ...
        <div style={{ marginBottom: "16px" }}>
          <ModalPostSelector
            ...
            label={<RequiredLabel text={__("Bundle Config", "wicket-memberships")} />}
            placeholder={__("Bundle Config", "wicket-memberships")}
            modalTitle={__("Select Bundle Config", "wicket-memberships")}
            value={form.bundleConfig}
            onChange={set("bundleConfig")}
            loadOptions={loadBundleConfigs}
            columns={[
          ...
          />
          {errors.bundleConfig && <p style={{ color: "#cc1818", margin: "4px 0 0" }}>{errors.bundleConfig}</p>}
        </div>
...
const CreateMembershipBundlePage = ({ bundleConfigCptSlug, listUrl, editBundleBaseUrl }) => {
  ...
  return (
    <AppWrap>
      <div className="wrap">
        ...
        >
          <EditWrap>
            <CreateMembershipBundlePageContent
              key={errorBoundaryResetKey}
              bundleConfigCptSlug={bundleConfigCptSlug}
              listUrl={listUrl}
              editBundleBaseUrl={editBundleBaseUrl}
            />
          </EditWrap>
        </AdminPageErrorBoundary>
      </div>
...
export default CreateMembershipBundlePage;
