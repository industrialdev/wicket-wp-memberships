...
import { fetchMdpPersons, updateBundleChangeOwnership } from "../../shared/services/api";

/**
 * MembershipBundleOwnerSection — bundle page adapter for MembershipOwnerSection.
 *
 * Maps bundle pageData to the flat props expected by the shared UI component and
 * injects the bundle-specific API call and copy.
 *
 * @param {object}       props
 ...
 */
const MembershipBundleOwnerSection = ({ pageData, isLoading, onOwnerUpdated }) => {
  if ( isLoading || ! pageData ) { return null; }

  const bundlePostId = pageData.ID;
  ...
  const loadOptions = (inputValue, callback) => {
    ...
    fetchMdpPersons({ term: inputValue })
      ...
      .catch((error) => {
        console.error('[MembershipBundleOwnerSection] loadOptions error', error);
      });
  };
  ...
  const handleSave = async (selectedOption) => {
    ...
    const response = await updateBundleChangeOwnership(bundlePostId, selectedOption.value);
  ...
  return (
    <MembershipOwnerSection
      title={__('Membership Bundle Owner', 'wicket-memberships')}
      tooltipText={__('Represents the person responsible for managing and renewing this Membership Bundle.', 'wicket-memberships')}
...
export default MembershipBundleOwnerSection;
