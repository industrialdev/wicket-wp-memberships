...
import { cancelMembershipBundle } from "../../shared/services/api";
 ...
 *
 * Opened when an admin sets a membership bundle status to "cancelled".
 ...
 */
const CancelMembershipBundleModal = ({
  isOpen,
  bundlePostId,
  onRequestClose,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await cancelMembershipBundle(bundlePostId, data);
      resetState();
      onSuccess(result?.success ?? __("Membership bundle cancelled successfully.", "wicket-memberships"));
    } catch (err) {
  ...
  return (
    <WicketModal
      isOpen={isOpen}
      title={__("Cancel Membership Bundle", "wicket-memberships")}
    ...
    >
      ...
      <p>
        {__("Cancelling this membership bundle is permanent and cannot be undone. Choose how individual memberships should be handled before proceeding.", "wicket-memberships")}
      </p>
        ...
        <div style={{ marginBottom: "16px" }}>
          <SelectControl
            ...
            options={[
              ...
              { label: __("Cancel at membership bundle end date", "wicket-memberships"), value: TIMING_AT_END_DATE },
            ]}
...
export default CancelMembershipBundleModal;
