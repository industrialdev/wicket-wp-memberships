...
- `CURRENT_SCOPE.md`: active product-scope document for features in development. Read this before working on membership bundle features.
...
`CURRENT_SCOPE.md` describes the **Membership Bundles** feature currently in development. Key concepts:
- A **Membership Bundle** is a container (WP custom post) that holds individual membership records and is linked to an MDP organisation.
- A **Membership Bundle Config** defines date calculation, renewal windows, grace periods, and renewal type for groups.
- Membership Bundles have their own WooCommerce subscription; individual memberships within a group are represented as line items on that subscription.
- Admin flows covered: Create Group, Add/Remove/Move members, Bulk CSV import, Cancel Group, and detail/list views.

Read `CURRENT_SCOPE.md` in full before modifying anything related to membership bundles or the group subscription model.
...
- If the change touches membership bundle logic, was `CURRENT_SCOPE.md` consulted first?
