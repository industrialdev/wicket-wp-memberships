...
function add_order_item_meta ( $item_id, $values ) {
  ...
  public function get_membership_callouts( $user_id = null ) {
    ...
    foreach( $memberships as &$membership) {
      ...
      // Reset per-iteration state explicitly. Without these resets, values set while
      // processing one membership (e.g. a pending_approval callout's 'email' key, an
      // autopay flag from a previous subscription, or a stale renewal URL) bleed into
      // the next iteration and produce malformed callouts or wrong payment links.
      $callout = [];
      $membership_data = [];
      $renewal_link_url = '';
      ...
      // A tier without a config is a data-integrity issue. Calling methods on a null
      // object would throw a fatal error and kill the callout loop for the entire user,
      // silencing all subsequent memberships. Skip this record and process the rest.
...
#        continue;
      ...
      if(!empty($next_tier_subscription_renewal)) {
        // Initialise to null so the stale-order guard below ($the_order check before
        // calling get_checkout_payment_url) behaves correctly when $renewal_orders is empty.
        $the_order = null;
        ...
        // A missing or deleted subscription would cause every subsequent method call
        // on $current_subscription to fatal. Skip this membership and continue.
        ...
        foreach ($renewal_orders as $order_id) {
          ...
          // 'failed' is included alongside 'pending': a payment that failed while the
          // subscription is on-hold still needs a checkout URL so the member can retry.
        ...
        if( empty($renewal_link_url) && !empty( $the_order) ) {
          ...
          foreach ($renewal_orders as $order_id) {
            ...
            if(!empty($parent_order)) {
              // $order_id is an integer (get_related_orders returns IDs, not objects).
              // Previously this called $order_id->get_id() which threw a fatal error.
              $parent_order->add_order_note("Subscription renewal order ID#".$order_id." generated by Membership Plugin.");
            }
            break;
          }
          $renewal_link_url = $the_order->get_checkout_payment_url();
        } else if( empty($renewal_link_url) ) {
          // Only fall back to the early-renewal URL when no checkout URL was already
          // resolved above (pending/failed order). The original else-branch ran
          // unconditionally, overwriting a valid checkout URL with the early-renewal link.
          $renewal_link_url = wcs_get_early_renewal_url( $current_subscription );
        }
      ...
      if( $current_time >= $membership_early_renew_at && $current_time < $membership_ends_at ) {
        // Only suppress the callout when a live subscription is handling autopay.
        // $is_autopay_enabled was reset to false at the top of this iteration, so
        // memberships without a subscription_id naturally fall through and show the callout.
        // Previously the $early_renewal[] assignment lived inside the subscription block,
        // which meant non-subscription memberships never received a renewal callout at all.
