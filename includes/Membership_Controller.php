...
#        continue;
      ...
      if(!empty($next_tier_subscription_renewal)) {
        ...
        if( empty($renewal_link_url) && strtotime($membership_data['meta']['membership_ends_at']) > $current_time ) {
          ...
          $renewal_link_url = $this->create_subscription_renewal_invoice_link( $current_subscription, $membership_data );
  ...
  protected function create_subscription_renewal_invoice_link( $current_subscription, array $membership_data ) {
    try {
      ...
      $this->log_subscription_renewal_issue(
        'Unable to generate subscription renewal invoice.',
    ...
    foreach ( $renewal_orders as $order_id ) {
      ...
      $parent_order = !empty( $membership_data['meta']['membership_parent_order_id'] ) ? wc_get_order( $membership_data['meta']['membership_parent_order_id'] ) : null;
      if ( ! empty( $parent_order ) ) {
        $parent_order->add_order_note( 'Subscription renewal order ID#' . $renewal_order->get_id() . ' generated by Membership Plugin.' );
      }
    ...
    $this->log_subscription_renewal_issue(
      'Subscription renewal order generation completed without a renewal order.',
      $current_subscription,
      $membership_data
    );
  ...
  protected function log_subscription_renewal_issue( $message, $current_subscription, array $membership_data, array $extra_context = [] ) {
    $context = array_merge(
      [
        'subscription_id' => method_exists( $current_subscription, 'get_id' ) ? $current_subscription->get_id() : '',
        'subscription_status' => method_exists( $current_subscription, 'get_status' ) ? $current_subscription->get_status() : '',
        'membership_id' => !empty( $membership_data['ID'] ) ? $membership_data['ID'] : '',
        'membership_subscription_id' => !empty( $membership_data['meta']['membership_subscription_id'] ) ? $membership_data['meta']['membership_subscription_id'] : '',
        'membership_parent_order_id' => !empty( $membership_data['meta']['membership_parent_order_id'] ) ? $membership_data['meta']['membership_parent_order_id'] : '',
      ],
      $extra_context
    );
