...
/**
 * Membership Bundle Config CPT Hooks
 *
 * Provides admin list customisation and a React-based create/edit page
 * for the wicket_mship_bcfg post type. Mirrors the structure of
 * Membership_Config_CPT_Hooks for the standard membership config CPT.
 */
class Membership_Bundle_Config_CPT_Hooks {

  const EDIT_PAGE_SLUG = 'wicket_mship_bcfg_edit';

  private $bundle_config_cpt_slug = '';

  public function __construct() {
    $this->bundle_config_cpt_slug = Helper::get_membership_bundle_config_cpt_slug();

    if ( ! empty( $_ENV['WICKET_MSHIP_ENABLE_BUNDLES'] ) ) {
      add_action( 'admin_menu', [ $this, 'add_bundle_configs_submenu' ], 28 );
      add_action( 'admin_menu', [ $this, 'add_edit_page' ] );
    }
    add_action( 'admin_init', [ $this, 'create_edit_page_redirects' ] );
    add_action( 'admin_enqueue_scripts', [ $this, 'enqueue_scripts' ] );
    add_action( 'rest_api_init', [ $this, 'register_rest_fields' ] );
    add_action( 'manage_' . $this->bundle_config_cpt_slug . '_posts_columns', [ $this, 'table_head' ] );
    add_action( 'manage_' . $this->bundle_config_cpt_slug . '_posts_custom_column', [ $this, 'table_content' ], 10, 2 );

    add_filter( 'post_row_actions', [ $this, 'row_actions' ], 10, 2 );

    // Skip trash for bundle configs
    add_action( 'trashed_post', [ $this, 'directory_skip_trash' ] );

    // Prevent moving to trash if the config is in use by a membership bundle
    add_filter( 'pre_trash_post', [ $this, 'prevent_trash' ], 10, 2 );
  }

  /**
   * Prevent trashing a bundle config that has associated membership bundles.
   */
  public function prevent_trash( $trash, $post ) {
    if ( $this->bundle_config_cpt_slug === $post->post_type ) {
      $linked_bundles = get_posts( [
        'post_type'      => Helper::get_membership_bundle_cpt_slug(),
        'posts_per_page' => 1,
        'meta_key'       => 'membership_bundle_config_id',
        'meta_value'     => $post->ID,
        'fields'         => 'ids',
      ] );

      if ( ! empty( $linked_bundles ) ) {
        wp_die( __( 'This bundle configuration is in use by one or more membership bundles. It cannot be moved to trash.', 'wicket-memberships' ) );
      }
    }
  ...
  /**
   * Force-delete instead of trashing bundle config posts.
   */
  public function directory_skip_trash( $post_id ) {
    if ( get_post_type( $post_id ) === $this->bundle_config_cpt_slug ) {
  ...
  /**
   * Register the Bundle Configs submenu entry pointing to the CPT list screen.
   */
  public function add_bundle_configs_submenu() {
    add_submenu_page(
      WICKET_MEMBERSHIP_PLUGIN_SLUG,
      esc_attr__( 'Bundle Configs', 'wicket-memberships' ),
      esc_attr__( 'Bundle Configs', 'wicket-memberships' ),
      Wicket_Memberships::WICKET_MEMBERSHIPS_CAPABILITY,
      'edit.php?post_type=' . $this->bundle_config_cpt_slug
    );
  }
  ...
  public function render_page() {
    $bundle_config_list_page_url = admin_url( 'edit.php?post_type=' . $this->bundle_config_cpt_slug );
    ...
    echo <<<HTML
      <div
        ...
        data-bundle-config-cpt-slug="{$this->bundle_config_cpt_slug}"
        data-bundle-config-list-url="{$bundle_config_list_page_url}"
  ...
  public function create_edit_page_redirects() {
    ...
    if ( $pagenow === 'post-new.php' && isset( $_GET['post_type'] ) && $_GET['post_type'] === $this->bundle_config_cpt_slug ) {
    ...
    if (
      ...
      if ( $post_type === $this->bundle_config_cpt_slug ) {
  ...
  /**
   * Enqueue the React bundle only on the bundle config edit page.
   */
  public function enqueue_scripts() {
    ...
    $asset_file = include( WICKET_MEMBERSHIP_PLUGIN_DIR . 'frontend/build/membership_bundle_config_create.asset.php' );

    wp_enqueue_script(
      WICKET_MEMBERSHIP_PLUGIN_SLUG . '_membership_bundle_config_create',
      WICKET_MEMBERSHIP_PLUGIN_URL . '/frontend/build/membership_bundle_config_create.js',
   ...
   * bundle config CPT so the frontend selector can display them without a
   * separate endpoint.
   */
  public function register_rest_fields() {
    register_rest_field(
      $this->bundle_config_cpt_slug,
      'cycle_type',
      [
        'get_callback' => function( $post_arr ) {
          $config = new Membership_Bundle_Config( $post_arr['id'] );
          return (string) $config->get_cycle_type();
        },
        'schema' => [
          'type'        => 'string',
          'description' => 'Cycle type for this bundle config.',
          'context'     => [ 'view' ],
        ],
      ]
    );
    ...
    register_rest_field(
      ...
      [
        ...
        'schema' => [
          'type'        => 'string',
          'description' => 'Renewal type for this bundle config.',
          'context'     => [ 'view' ],
        ],
      ]
    );
  ...
  /**
   * Add custom columns to the bundle config list table.
   */
  public function table_head( $columns ) {
    ...
    if ( ! empty( $_ENV['WICKET_MEMBERSHIPS_DEBUG_MODE'] ) ) {
      ...
      $columns['bundle_config_data']    = __( 'Bundle Config Data', 'wicket-memberships' );
    }
  ...
  /**
   * Populate custom column cells in the bundle config list table.
   */
  public function table_content( $column_name, $post_id ) {
    ...
    if ( $column_name === 'renewal_window_data' ) {
      ...
      echo '</pre>';
    } elseif ( $column_name === 'bundle_config_data' ) {
      echo '<pre>';
      print_r( get_post_meta( $post_id, 'bundle_config_data', true ) );
      echo '</pre>';
    }
  }
}
